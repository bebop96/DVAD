...
class ConvBNSSMBlock(nn.Module):
	def __init__(
		...
		self.proj = nn.Conv2d(in_channels=hidden_dim * 2, out_channels=hidden_dim, kernel_size=1, stride=1)
  
		# MOE setup
		self.num_experts = 5
		self.experts = nn.ModuleList([
			nn.Conv2d(in_channels=hidden_dim * 2, out_channels=hidden_dim, kernel_size=1, stride=1) 
			for _ in range(self.num_experts)
		])
		self.gating_network = nn.Conv2d(in_channels=hidden_dim * 2, out_channels=self.num_experts, kernel_size=1, stride=1)
		self.adapters = Adapter(input_dim=hidden_dim, latent_dim=64)
	...
	def forward(self, input: torch.Tensor):
		...
		for blk in self.smm_blocks:
			out_ssm = blk(out_ssm)
		# combined_output = out_ssm.permute(0, 3, 1, 2).contiguous()  # (batch_size, hidden_dim * 3, H, W)
		...
		combined_output = torch.cat((out_ssm.permute(0, 3, 1, 2).contiguous(), out_55, out_77), dim=1)  # (batch_size, hidden_dim * 3, H, W)
		combined_output = torch.cat((out_ssm.permute(0, 3, 1, 2).contiguous(), out_55, out_77), dim=1)  # (batch_size, hidden_dim * 3, H, W)
		
		# hyper
		input_conv = input.permute(0, 3, 1, 2).contiguous()
		dynamic_out=self.adapters(input_conv)
		# combined_output = dynamic_out  # (batch_size, hidden_dim * 3, H, W)
		# combined_output = torch.cat((out_55, out_77, dynamic_out), dim=1)  # (batch_size, hidden_dim * 3, H, W)
		# combined_output = torch.cat((out_ssm.permute(0, 3, 1, 2).contiguous(), dynamic_out), dim=1)  # (batch_size, hidden_dim * 3, H, W)
		combined_output = torch.cat((out_ssm.permute(0, 3, 1, 2).contiguous(), out_55, out_77, dynamic_out), dim=1)  # (batch_size, hidden_dim * 3, H, W)

		gate_output = self.gating_network(combined_output)  # (batch_size, num_experts, H, W)
		gate_output = F.softmax(gate_output, dim=1)  # Softmax over experts
		expert_outputs = torch.stack([expert(combined_output) for expert in self.experts], dim=1)  # (batch_size, num_experts, hidden_dim, H, W)
		weighted_output = torch.sum(gate_output.unsqueeze(2) * expert_outputs, dim=1).permute(0, 2, 3, 1).contiguous()  # (batch_size, H, W, hidden_dim)

		# return output + input
		return weighted_output + input  # Skip connection as in the original block
...
class MFF_OCE(nn.Module):
	...
	def forward(self, x):
		fpn0 = self.relu(self.bn1(self.conv1(x[0]))) # (1,64,64,64)->(1,128,32,32)
		fpn1 = self.relu(self.bn21(self.conv21(x[1]))) + fpn0 # (1,128,32,32)->(1,128,32,32)
		sv_features = self.relu(self.bn2(self.conv2(fpn1))) + self.relu(self.bn31(self.conv31(x[2]))) # (1,128,32,32)|(1,2546,16,16)->(1,256,16,16)
		sv_features = self.relu(self.bnf(self.convf(sv_features))) # (1,256,16,16)
		sv_features = self.bn_layer(sv_features) # (1,512,8,8)
...
class MAMBAAD(nn.Module):
	def __init__(self, model_t, model_s):
		...
		# self.adapters = nn.ModuleList([
      	# 	Adapter(input_dim=64, hidden_dim=32, num_classes=15, class_embed_dim=1280),
      	# 	Adapter(input_dim=128, hidden_dim=32, num_classes=15, class_embed_dim=1280),
      	# 	Adapter(input_dim=256, hidden_dim=16, num_classes=15, class_embed_dim=1280),
        # ])
  
		# for name, param in self.adapters.named_parameters():
		# 	param.requires_grad = True
  
		self.frozen_layers = ['net_t','mff_oce']
	...
	# def train(self, mode=True):
	# 	"""
	# 	Set the model to training mode, freezing layers except for 'gating_layer' and 'experts' inside 'layers_up'.
	# 	"""
	# 	self.training = mode
	# 	for mname, module in self.named_children():
	# 		if mname == 'net_s':
	# 			# For net_s, iterate over the submodules
	# 			for sub_mname, sub_module in module.named_children():
	# 				if sub_mname == 'layers_up':
	# 					# Inside 'layers_up', freeze all layers except 'gating_layer' and 'experts'
	# 					for idx, inner_module in enumerate(sub_module.children()):
	# 						if isinstance(inner_module, nn.ModuleList):  # Check if it's blocks list
	# 							for block_idx, block in enumerate(inner_module):
	# 								# Within blocks, freeze all except experts and gating_layer
	# 								for param_name, param in block.named_parameters():
	# 									# Keep 'experts' and 'gating_layer' parameters trainable
	# 									if "adapters" in param_name:
	# 									# if "gating_network" in param_name or "experts" in param_name or "adapters" in param_name:
	# 										param.requires_grad = True
	# 									else:
	# 										param.requires_grad = False
	# 								block.train(mode)  # Set block to train mode
	# 						else:
	# 							# If the submodule is not a block (like a gating_layer or expert), leave it trainable
	# 							for param_name, param in inner_module.named_parameters():
	# 								if "adapters" in param_name:
	# 								# if "gating_network" in param_name or "experts" in param_name or "adapters" in param_name:
	# 									param.requires_grad = True
	# 								else:
	# 									param.requires_grad = False
	# 							inner_module.train(mode)
	# 				else:
	# 					# For other submodules within net_s that are not 'layers_up', freeze them
	# 					self.freeze_layer(sub_module)
	# 		elif mname in self.frozen_layers:
	# 			self.freeze_layer(module)  # Freeze layers in 'net_t' or other frozen layers
	# 		else:
	# 			module.train(mode)  # Keep other layers in train mode
	# 	return self
	...
	def forward(self, imgs):
		...
		feats_s = self.net_s(self.mff_oce.forward(x=feats_t))
		# feats_s = self.net_s(self.mff_oce.forward(x=feats_t, adapters=self.adapters))
...
class MambaUPNet(nn.Module):
	...
	def forward(self, x, adapters=None):
		x = rearrange(x,'b c h w -> b h w c') # 0-(1, 8, 8, 512); 1-(1, 16, 16, 256); 2-(1, 32, 32, 128); 3-(1, 64, 64, 64)
		out_features = []
		for i, layer in enumerate(self.layers_up):
			x = layer(x)
   
			# if i == 0:
			# 	dynamic_out = adapters[i](x, class_id=None)
			# 	scale=1e-3 * x.max()/dynamic_out.max()
			# 	x = x + dynamic_out * scale

			if i != 0:
				# dynamic_out = adapters[i-1](x, class_id=None)
				# x = x + dynamic_out
...
class Adapter(nn.Module):
	"""Conventional Adapter layer, using SwiGLU as the activation function."""

	def __init__(self, input_dim, latent_dim, num_classes=15, class_embed_dim=1280):
		super().__init__()
		...
		# self.down_sampler = nn.Linear(self.input_dim, self.hidden_dim)  # Double size for gating
		self.down_embed=Embedding(z_dim=16, in_dim=input_dim)
		self.down_hyper=HyperNetwork(in_size=input_dim, z_dim=16, out_size=latent_dim)


		# self.up_sampler = nn.Linear(self.hidden_dim//2 , self.input_dim)
		self.up_embed=Embedding(z_dim=16, in_dim=latent_dim//2)
		self.up_hyper=HyperNetwork(in_size=latent_dim//2, z_dim=16, out_size=input_dim)
	...
	def swiglu(self, x):
		...
		x1, x2 = torch.chunk(x, 2, dim=-1)  # Split into two equal parts
		return silu(x1) * x2

	def forward(self, input, class_id=None):
		b,c,w,h=input.shape
		x=input.reshape(b, w*h, c)
		if class_id is None:
			down_w, down_b = self.down_embed(self.down_hyper, x)
		else:
			class_embed = self.class_embedding(class_id)  # [batch_size, class_embed_dim]
			if len(class_embed.shape)==1:
				class_embed=class_embed[None]
			class_embed = class_embed.unsqueeze(dim=1)
			down_w, down_b = self.down_embed(self.down_hyper, torch.cat([x, class_embed], dim=1))

		z = torch.matmul(x, down_w) + down_b # torch.Size([16, 256, 16])
		z = self.swiglu(z)  # Apply SwiGLU

		# z = self.up_sampler(z)
		up_w, up_b = self.up_embed(self.up_hyper, z)
		z = torch.matmul(z, up_w) + up_b # torch.Size([16, 256, 1280])
...
class Embedding(nn.Module):
    ...
    def __init__(self, z_dim=256, in_dim=1280):
        super(Embedding, self).__init__()
        self.z_dim = z_dim
        self.in_dim=in_dim
        self.bn = nn.BatchNorm1d(self.in_dim)
        self.linear = nn.Linear(self.in_dim, self.z_dim)
        

    def forward(self, hypers, tokens):
        shape=tokens.shape
        tokens = tokens.reshape(-1, self.in_dim) # (4096, 1280)
        tokens = self.bn(tokens) # (N, 2048)
        tokens = F.relu(self.linear(tokens.view(-1, self.in_dim))) # downsampling, (4096, z_dim=256)
        w, b = hypers(torch.mean(tokens, dim=0).view(-1, self.z_dim)) # (N, z_dim) -> (z_dim), w: (1280, 1280), b: (1,1280)
        return w, b 
    
    
class HyperNetwork(nn.Module):

    def __init__(self, in_size, z_dim, out_size):
        super(HyperNetwork, self).__init__()
        self.z_dim = z_dim
        self.out_size = out_size
        self.in_size = in_size

        self.w1 = Parameter(torch.Tensor(self.z_dim, self.out_size * self.in_size), requires_grad=True)
        self.b1 = Parameter(torch.Tensor(self.out_size * self.in_size), requires_grad=True)
        self.w2 = Parameter(torch.Tensor(self.z_dim, self.out_size), requires_grad=True)
        self.b2 = Parameter(torch.Tensor(self.out_size), requires_grad=True)
